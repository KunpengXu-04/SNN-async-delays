...
**Mechanism master figure (L5, use this for presentations/paper):**
`runs/vis_burst_spkout/mechanism_figure/burst_mechanism_K{1,2}.png`, generated by
`python -m scripts.plot_burst_mechanism` (reads `diagnostic_data.npz`, no re-run).
Focused wad|d0 side-by-side raster: only firing spikes + the `d_ho` delay arcs that
route each hidden spike's *arrival* into the readout window. K≥2 shows the K temporal
slots (query 0/1…). This replaces the dense weight-fan flow below, which buries the
message under thousands of faint edges.

**Per-run mechanism figure (every sweep cell):** shared core `utils.viz.draw_mechanism_on_ax`
renders it both (a) into the **diagnostic panel's bottom-right** (replacing the old dense
layer-to-layer spike flow) and (b) as a standalone `plots/mechanism_sample0.png`. Generated
automatically by `run_plan_d_h_sweep.py`; standalone/backfill via
`python -m scripts.plot_burst_mechanism --run_dir <run>` or `--runs_dir <folder>`.
**Spike-based, with a no-spike fallback:**
- spiking-output runs (`d_ho` + output spikes): each HIDDEN spike → output arrival via `d_ho`;
- MLP-readout runs (only `d_ih`, e.g. compress sweeps) **with hidden spikes**: draw each hidden
  spike (green = fired in readout window, blue = before) plus up to 4 causal input→hidden arcs
  (input arrivals within a membrane window before the spike). Only actual spikes drive arcs;
- **only when the hidden layer fires ZERO spikes** (e.g. burst @ low h, `spk/tr`≈0.2) does it
  fall back to the full input→hidden `d_ih` ARRIVAL routing (top-N |w| per input spike) so the
  figure is never empty.
The runner deletes `layer_to_layer_spike_flow_sample0.png` in favor of this.

Legacy (superseded): `plots/layer_to_layer_spike_flow_sample0.png` (input→hidden→output
with weight-coloured fan edges). NOT `enhanced_flow.png` (only shows input→hidden by design).
