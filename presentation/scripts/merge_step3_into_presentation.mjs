...
const PRESENTATION_ROOT = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..");
const BASE_PATH = path.join(PRESENTATION_ROOT, "current", "presentation.pptx");
const STEP3_PATH = path.join(PRESENTATION_ROOT, "generated", "presentation_step3_update.pptx");
const BACKUP_PATH = path.join(PRESENTATION_ROOT, "archive", "presentation_before_step3_merge_backup.pptx");

// Insert the 3 new Step 3 slides right after the Steps 1-2 "Conclusions" slide
// (index 10, 0-based -> the 11th slide) and before the Step 4 title slide
// (index 11, 0-based -> the 12th slide).
const INSERT_AFTER_INDEX = 10;
...
function buildInsertedSlides(targetProto, sourceProto, slides, tag) {
...
const [baseProto, step3Proto] = await Promise.all([
  loadDeckProto(BASE_PATH),
  loadDeckProto(STEP3_PATH),
]);
...
const insertedSlides = buildInsertedSlides(merged, step3Proto, step3Proto.slides, "step3");

merged.slides.splice(INSERT_AFTER_INDEX + 1, 0, ...insertedSlides);
merged.slides.forEach((slide, idx) => {
  slide.index = idx;
});
