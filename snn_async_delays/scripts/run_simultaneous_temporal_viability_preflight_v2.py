"""Run the six locked held-out cells of temporal viability preflight v2."""
...
def main() -> None:
    ...
    if prereg.get("status") != "preregistered":
        raise SystemExit("Temporal viability preflight v2 is not preregistered")
    base = yaml.safe_load(
        (BASE / "configs/simultaneous_temporal_routing_pilot_v1.yaml")
        .read_text(encoding="utf-8")
    )
    root = BASE / "runs/exploratory" / prereg["protocol_id"]
    print(f"{prereg['protocol_id']}: {prereg['cells']} locked held-out cells")
    for seed in prereg["held_out_execution_seeds"]:
        for condition in prereg["conditions"]:
            ...
            protocol["loss_reduction"] = prereg["loss_reduction"]
            protocol["training"] = copy.deepcopy(prereg["training"])
            protocol["fixed_matched_delay_range"] = prereg["fixed_full_range"]
