"""
Step 2 – Same op, K queries per trial; sweep K to find max-K @ accuracy >= tau.

Single run:
    python -m scripts.run_step2 --config configs/step2_multiquery_sameop.yaml
    python -m scripts.run_step2 --config configs/step2_multiquery_sameop.yaml \\
        --K 4 --train_mode weights_only

Sweep K across all train_modes:
...
def run_single(cfg: dict, K: int, device: str, base_runs_dir: str) -> dict:
    ...
    mode     = cfg["train_mode"]
    h        = cfg["hidden_size"]
    run_name = f"step2_{op}_{mode}_h{h}_K{K}_seed{cfg['seed']}"
    ...
    results.update({"op": op, "train_mode": mode, "hidden_size": h, "K": K})
