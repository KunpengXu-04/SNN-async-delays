"""
Timing-Parameter Ablation for Plan D Sequential Temporal Multiplexing
(Direction B: investigating the Max K@90% capacity ceiling).

Fixed architecture (L1-h50 + MLP readout + trainable delays); varies
sub_win / read_len / lif_tau_m across 5 conditions x K x seeds, then
writes a combined summary CSV/JSON and comparison plots.

Usage:
    python -m scripts.run_timing_ablation \\
        --config configs/step2_timing_ablation.yaml --device cuda
    python -m scripts.run_timing_ablation --config configs/step2_timing_ablation.yaml \\
        --K 3 --condition baseline --seed 42 --device cuda
"""
...
def run_single(
    ...
    """Train and evaluate one (timing_condition, K, seed) combination."""
    set_seed(seed)
    logger = setup_logger("timing_ablation")

    op        = base_cfg["op_name"]
    cname     = condition["name"]
    sub_win   = condition["sub_win"]
    read_len  = condition["read_len"]
    lif_tau_m = condition["lif_tau_m"]
    win_len   = K * sub_win
    d_max     = win_len
    ...
    run_name  = f"{cname}_K{K}_seed{seed}"
    ...
    logger.info(
        ...
        f"read_len={read_len}, tau_m={lif_tau_m}) ==="
    )
    ...
    save_run_diagnostic_plots(
        model=model,
        cfg={**run_cfg, "model_name": mname, "condition": cname,
             "hidden_sizes": hidden_sizes,
             "readout_type": readout_type,
             "num_hidden_layers": num_layers},
        log_rows=log_rows,
    ...
    logger.info(
        f"K={K}  cond={cname}  acc={results['accuracy']:.4f}  "
...
def _write_summary_csv(all_results: list[dict], path: str):
    ...
    cols = [
        "condition", "sub_win", "read_len", "lif_tau_m",
        "model_name", "readout_type", "train_mode", "K", "seed",
...
def main():
    ...
    parser.add_argument("--config",    default="configs/step2_timing_ablation.yaml")
    parser.add_argument("--K",         default=None, type=int, help="Single K override")
    parser.add_argument("--condition", default=None, help="Single condition name override")
    ...
    runs_dir = args.runs_dir or os.path.join(base, "runs", "timing_ablation")

    sweep = base_cfg.get("sweep", {})
    K_vals     = [args.K] if args.K is not None else sweep.get("K_values", [3, 4])
    seeds      = [args.seed] if args.seed is not None else sweep.get("seeds", [42, 0])
    model_cfg  = sweep.get("model", {})
    conditions = sweep.get("conditions", [])
    if args.condition is not None:
        conditions = [c for c in conditions if c["name"] == args.condition]
        if not conditions:
            print(f"Condition '{args.condition}' not found in config sweep.conditions")
            return
    ...
    for condition in conditions:
        for K in K_vals:
            for seed in seeds:
                ...
                res = run_single(cfg, model_cfg, condition, K, seed, device, runs_dir)
    ...
    csv_path  = os.path.join(runs_dir, "timing_ablation_summary.csv")
    json_path = os.path.join(runs_dir, "timing_ablation_summary.json")
    ...
    # ── Aggregated table ──
    print(f"\n{'='*78}")
    print("Timing Ablation Summary — Plan D Sequential (NAND, L1-h50 + MLP readout)")
    print(f"{'='*78}")
    print(f"{'Condition':14s}  {'K':>3}  {'Acc (mean±range)':>20}  {'K/spk':>8}")
    print("-" * 70)

    from collections import defaultdict
    grouped: dict = defaultdict(list)
    for r in all_results:
        grouped[(r.get("condition", "?"), r.get("K", 0))].append(r)

    for (cname, K), runs in sorted(grouped.items(), key=lambda x: (x[0][0], x[0][1])):
        ...
        spks = [r["throughput_K_per_spk"] for r in runs
                ...
                and not (r["throughput_K_per_spk"] != r["throughput_K_per_spk"])]
        if accs:
            acc_str = f"{sum(accs)/len(accs)*100:.1f}% ± {(max(accs)-min(accs))*100:.1f}%"
        else:
            acc_str = "n/a"
        spk_str = f"{sum(spks)/len(spks):.3f}" if spks else "n/a"
        print(f"{cname:14s}  {K:>3}  {acc_str:>20}  {spk_str:>8}")

    # Max K@tau by condition
    print()
    conds_all = {r.get("condition") for r in all_results}
    for cname in sorted(conds_all):
        c_runs = [r for r in all_results if r.get("condition") == cname]
        ...
        for tau in tau_list:
            ...
            print(f"  {cname:14s}  tau={tau:.0%}  max_K={mk}")
